...
class AI:
    ...
    def _sentiment_fallback(self, symbol: str, signal_time: datetime, items: list[dict[str, Any]], ai_error: str):
        ...
        # An unreadable article is unresolved evidence. Require all selected unique
        # links to succeed so partial evidence cannot silently authorize a trade.
        if failures:
            error = "ARTICLE_FALLBACK_INCOMPLETE"
            self._log_external("SENTIMENT_FALLBACK", symbol, "ERROR", "Incomplete article coverage",
                               _j({"ai_error": ai_error, "failures": failures}), [])
            return "ERROR", "Incomplete article-body sentiment", error
        title = " ".join(str(best_item.get("title") or "Article body").split()[:8])
        hint = f"Article fallback: {title}"
    ...
    def _classify_yahoo_news(self, symbol: str, direction: str, signal_time: datetime, price: float, items: list[dict[str, Any]]):
        ...
        if cls not in {"POSITIVE", "NEGATIVE", "NEUTRAL", "UNRELATED"}:
            return self._sentiment_fallback(symbol, signal_time, items, "MALFORMED_YAHOO_CLASSIFICATION")
        if not hint:
