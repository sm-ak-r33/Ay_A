...
# Local deterministic fallback for Yahoo headline classification.
...
# not used. No secrets or external API calls are required for this fallback.
...
class AI:
    def __init__(self, key: str, cfg: Cfg, gs: GS):
        ...
        # The finance phrase layer below remains available even if the package import
        # fails, so an installation issue still cannot kill S1/S3 classification.
        self.sentiment = None
        try:
    ...
    def _finance_phrase_score(title: str) -> tuple[float, list[str]]:
        """Deterministic catalyst score for financial-news headlines.
    ...
    def _sentiment_fallback(self, symbol: str, signal_time: datetime, items: list[dict[str, Any]], ai_error: str):
        """Local fallback used only when the LLM headline classifier fails.

        It combines VADER's deterministic headline sentiment with finance-specific
        catalyst phrases. The finance phrases dominate the score; generic sentiment
        alone must be strong to produce a directional classification.
        """
        ...
        for item in items[:6]:
            ...
            if not title:
                continue
            phrase_score, matches = self._finance_phrase_score(title)
            vader = 0.0
            if self.sentiment is not None:
                try:
                    vader = float(self.sentiment.polarity_scores(title).get("compound", 0.0))
                except Exception:
                    vader = 0.0
            ...
            # Domain phrase evidence is primary. Generic VADER can reinforce it. If
            # there is no finance phrase, only very strong generic sentiment matters.
            if matches:
                ...
                if vader >= 0.65:
                    ...
                    generic = 1.35 * vader
                combined = generic * recency
            scored.append((combined, phrase_score, vader, title, matches, item))

        if not scored:
            cls = "NEUTRAL"
            hint = "Fallback found no scorable headline"
            diagnostic = f"ai_error={ai_error}; no_scorable_headlines"
            self._log_external("SENTIMENT_FALLBACK", symbol, cls, hint, diagnostic, [])
            return cls, hint, ""

        # Use the strongest catalyst, with recency already incorporated. This prevents
        # several bland headlines from washing out one genuinely material event.
        best = max(scored, key=lambda z: abs(z[0]))
        score, phrase_score, vader, title, matches, item = best
        if score >= FALLBACK_POS_THRESHOLD:
            cls = "POSITIVE"
        elif score <= FALLBACK_NEG_THRESHOLD:
            cls = "NEGATIVE"
        else:
            cls = "NEUTRAL"

        hint = " ".join(title.split()[:8])
        diag = (
            f"ai_error={ai_error}; score={score:.3f}; phrase={phrase_score:.3f}; "
            f"vader={vader:.3f}; matches={','.join(matches) if matches else 'none'}"
        )
        src = [str(item.get("link") or "")] if item.get("link") else []
        self._log_external("SENTIMENT_FALLBACK", symbol, cls, hint, diag, src)
        return cls, f"Fallback: {hint}", ""
    ...
    def signal(self, s: str, direction: str, t: datetime, p: float, zr: float, zv: float, vol: float, fresh: bool = False, policy: str = ""):
        ...
        If that classifier fails, a local VADER + finance-catalyst sentiment fallback
        makes a conservative decision. If Yahoo has no recent news, no LLM call is made.
        """
...
def live():
    ...
    if not ok:
        ...
        if _bool(cfg["ne"]):
            tg.send(f"[SYSTEM ERROR]\nAI_PREFLIGHT\nOpenRouter unavailable: {err[:300]}\nYahoo news lookup and the local sentiment fallback remain available. S1/S2/S3 can still classify retrieved headlines without OpenRouter if needed; S5 biotech/energy web research still needs OpenRouter. Nasdaq earnings fallback remains available.")
