...
class AI:
    ...
    def _call(self, kind, symbol, messages, max_tokens, web=True):
        ...
        if self.calls >= self.daily_limit:
            return None, f"AI_DAILY_LIMIT_{self.daily_limit}", []
        ...
        q = [
            {
                "role": "system",
                "content": (
                    "You are a strict market-news gate. Use current web results. Output exactly NO or "
                    "YES|a factual 5-6 word catalyst. YES requires a recent credible company/security-specific "
                    "catalyst that plausibly explains the move. Do not use generic market commentary."
                ),
            },
            {
                "role": "user",
                "content": (
                    f"Ticker {s}. Abnormal {move}. Time {t.isoformat()}. Price {p:.6f}. "
                    f"z-range {zr:.2f}; z-volume {zv:.2f}; volume {vol:.0f}. "
                    f"Search only the prior {hrs} hours."
                ),
            },
        ]
...
class Core:
        ...
        if direction == "U" and self.c.enabled(2):
            ...
            if arm:
                ...
                if 0 < dt <= delay:
                    ...
                    meta = {
                        ...
                        "second_alert_time_et": _et(z.end),
                    ...
                    self.jobs.put(AIJob("S2", sg2, meta))
            ...
            if not paired and z.s not in self.s2arm:
                ...
                self._trace(2, "FIRST_ALERT", z.s, "first upward anomaly armed; waiting up to 5 minutes", {
                    "trade_id": sid, "first_alert_time_et": _et(z.end), "first_alert_price": z.c
                })
    ...
    def _continue(self, b: M1):
        ...
        if self.c.enabled(st):
            ...
            self.jobs.put(AIJob("S1" if st == 1 else "S3", sg))
        else:
            self.sig.pop(sg.s, None)
            ...
            try:
                ...
                if kind == "S2":
                    ...
                    with self.lock:
                        ...
                        sg.sources = src
                        if dec == "ERROR":
                            self._trace(2, "AI_ERROR", sg.s, "news check unavailable; S2 does not short", {**meta, "error": err}, level="ERROR")
                            continue
                        if dec == "YES":
                ...
                with self.lock:
                    ...
                    st = 1 if sg.direction == "U" else 3
                    if dec == "ERROR":
                        self._trace(st, "AI_ERROR", sg.s, "AI catalyst check unavailable; no trade", {"signal_id": sg.id, "error": err}, level="ERROR")
                        self.sig.pop(sg.s, None)
                        continue
                    if sg.direction == "U":
