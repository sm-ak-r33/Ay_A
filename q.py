...
# High-quality research calls use Gemini through the SAME OpenRouter K07 key.
# No new secret or K08 field is required. The normal high-volume strategy
# classification path continues to use cfg["gm"] (for example openrouter/free).
...
class AI:
    ...
    def _call(self, kind, symbol, messages, max_tokens, web=True, response_format=None, model_override: Optional[str] = None):
        ...
        Use the server-side ``openrouter:web_search`` tool instead. Normal calls
        use cfg["gm"], while the small daily research jobs can pass a hard-coded
        Gemini model override without introducing any additional API secret.
        """
        ...
        selected_model = (model_override or str(self.cfg["gm"])).strip()
        body = {
    ...
    def _gemini_research_call(self, kind: str, symbol: str, messages, max_tokens: int, web: bool = True):
        """Run the small number of daily research calls on a fixed Gemini model.

        Both models use the existing OpenRouter K07 key. If the primary Gemini
        endpoint is temporarily unavailable, fall back to Gemini Flash Lite.
        High-volume S1/S2/S3 headline classification is intentionally NOT routed
        through this helper and therefore continues to use cfg["gm"].
        """
        last_err = ""
        for model in GEMINI_RESEARCH_MODELS:
            text, err, src = self._call(
                kind, symbol, messages, max_tokens, web=web, model_override=model
            )
            if text:
                return text, "", src, model
            last_err = f"{model}: {err or 'UNKNOWN_GEMINI_ERROR'}"
        return None, last_err or "GEMINI_RESEARCH_FAILED", [], ""
    ...
    def next_day_events(self, target: date, limit: int):
        ...
        text, err, _, _ = self._gemini_research_call("S5_EVENTS", "", q, 800, web=True)
        if not text:
            return [], err
        out = []
    ...
    def brief(self, today: date):
        ...
        text, _, _, _ = self._gemini_research_call("BRIEF", "", q, 900, web=True)
        return text or ""
...
def self_test():
    ...
    print(
        ...
        "POC utility/3R, S6 two-15m close-confirmed breakout + directional pullback + quarter-box stop, strategy-specific sheet schemas, Day_end_price formula, stock filter"
    )
