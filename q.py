...
def _price_tick(price: float) -> float:
    """Practical US-equity price increment used by the simulator."""
    return 0.01 if float(price) >= 1.0 else 0.0001


def _ceil_to_tick(price: float, tick: float) -> float:
    scale = round(1.0 / tick)
    return round(math.ceil(float(price) * scale - 1e-12) / scale, 4)
...
class AI:
    ...
    def _call(self, kind, symbol, messages, max_tokens, web=True, response_format=None, model_override: Optional[str] = None):
        ...
        body = {
            "model": model_override or self.cfg["gm"],
            "messages": messages,
            "temperature": 0,
        ...
        for host_i, url in enumerate(hosts):
            try:
                ...
                for x in rows:
                    ...
                    # Catalyst trading requires Yahoo itself to link the article to the
                    # exact ticker. Plain-text search hits are not enough (e.g. a company
                    # name that merely resembles a ticker/company name).
    ...
    def _classify_yahoo_news(self, symbol: str, direction: str, signal_time: datetime, price: float, items: list[dict[str, Any]]):
        """Classify already-retrieved Yahoo headlines with fixed non-router models.

        No web-search tool is used here. A deterministic fixed model is tried first;
        a second fixed free model is only used if the first request/provider fails.
        """
        ...
        q = [
            {
                "role": "system",
                "content": (
                    "Classify the already-retrieved Yahoo Finance headlines for the named ticker. "
                    "Do not browse. Return only the JSON required by the response schema. "
                    "classification must be POSITIVE, NEGATIVE, NEUTRAL, or UNRELATED. "
                    "Use POSITIVE/NEGATIVE only when the headline plausibly creates directional stock impact. "
                    "hint must be a short factual 3-8 word explanation."
                ),
            },
            {
                "role": "user",
        ...
        errors = []
        for model in AI_HEADLINE_MODELS:
            answer, err, _ = self._call(
                "YAHOO_CLASSIFY",
                symbol,
                q,
                AI_HEADLINE_MAX_TOKENS,
                web=False,
                response_format=schema,
                model_override=model,
            )
            if not answer:
                errors.append(f"{model}:{err or 'EMPTY'}")
                continue
            try:
                obj = json.loads(answer)
                cls = str(obj.get("classification", "")).strip().upper()
                hint = " ".join(str(obj.get("hint", "")).strip().split()[:8])
                ...
                errors.append(f"{model}:MALFORMED:{type(ex).__name__}")
                continue

        return "ERROR", "", " | ".join(errors)[:900] or "YAHOO_CLASSIFICATION_FAILED"
...
class Core:
    ...
    def _orb(self, b: M1):
        ...
        if q["armed"] and b.s not in self.pos[6]:
            ...
            entry = h1
            raw_stop_distance = S6_STOP_BOX_FRACTION * box_height
            tick = _price_tick(entry)

            # Keep the stop no farther away than 1/4 of the first box. For a long
            # position that means rounding the raw stop UP toward the entry.
            stop = _ceil_to_tick(entry - raw_stop_distance, tick)
            stop_distance = entry - stop
            if stop_distance < tick * 0.999:
                q["done"] = True
                self._trace(
                    6, "STOP_TOO_TIGHT", b.s,
                    "quarter-box stop is smaller than one practical price tick; setup skipped",
                    {
                        "range_high": q["h1"],
                        "range_low": q["l1"],
                    ...
                    once_key=f"{k}:tiny_stop",
                )
                return

            reward_r = float(self.c["s6r"])
            target = _ceil_to_tick(entry + reward_r * stop_distance, tick)

            # With one-minute OHLC we cannot know the within-bar order of entry vs
            # stop/target. If the retest candle already contains either exit level,
            # the trade outcome is unknowable, so skip instead of fabricating a win/loss.
            hit_stop_same_bar = float(b.l) <= stop
            hit_target_same_bar = float(b.h) >= target
            if hit_stop_same_bar or hit_target_same_bar:
                q["done"] = True
                self._trace(
                    6, "AMBIGUOUS_ENTRY_BAR", b.s,
                    "retest candle also crossed stop/target; intrabar sequence unknown, trade skipped",
                    {
                        "entry": entry,
                        "stop": stop,
                        "target": target,
                        ...
                        "retest_1m_low": b.l,
                        "retest_1m_close": b.c,
                        "hit_stop_same_bar": hit_stop_same_bar,
                        "hit_target_same_bar": hit_target_same_bar,
                    },
                    once_key=f"{k}:ambiguous_entry",
                )
                return
...
def _looks_like_stock(asset) -> bool:
    ...
    bad = [
        ...
        "ultrashort", "ultrapro", " fund", "fund ", "closed-end fund",
        "commodity pool", "warrant", " warrants", "right to purchase",
    ]
...
def self_test():
    ...
    assert AI_HEADLINE_MODELS[0] != "openrouter/free" and AI_HEADLINE_MODELS[1] != "openrouter/free"
    h1, l1 = 20.0, 19.2
    box = h1 - l1
    entry = h1
    ...
    # Practical tick rounding keeps the stop inside the 1/4-box maximum risk.
    e2, box2 = 39.095, 0.135
    raw2 = e2 - 0.25 * box2
    s2 = _ceil_to_tick(raw2, _price_tick(e2))
    assert s2 >= raw2 and s2 < e2 and round(s2, 2) == s2
    ...
    class C:
        ...
        tradable = True
    assert not _looks_like_stock(A()) and _looks_like_stock(B()) and not _looks_like_stock(C())

    print(
        "SELF_TEST_OK: aggregation, rolling stats, S1/S3 continuation, S2 two-alert timing, "
        "POC utility/3R, fixed Yahoo classifier models, S6 close-confirmed breakout + directional pullback + tick-rounded quarter-box stop + ambiguous-entry guard, strategy-specific sheet schemas, Day_end_price formula, stock/fund filter"
    )
