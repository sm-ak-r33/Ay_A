...
S6_SECOND_END = 10 * 60
# Exact ORB rules supplied by the user. No K08 changes are required.
S6_BREAKOUT_RANGE_FRACTION = 0.20   # H2 must exceed H1 by >20% of first-box height
S6_TOUCH_TOLERANCE = 0.05           # first post-10:00 1m candle within $0.05 of H1 qualifies
S6_STOP_BOX_FRACTION = 0.25         # stop = H1 - 25% of first-box height
S6_TARGET_BOX_MULTIPLE = 2.00        # target = H1 + 200% of first-box height
    ...
    def _manage_positions(self, b: M1):
        ...
        if z:
            if b.l <= z.stop:
                ...
                self._exit(6, b.s, z.target, t, "TWO_BOX_TARGET")

    def _orb(self, b: M1):
        """Strategy 6: exact two-candle opening-range logic supplied by the user.

        09:30-09:45 ET
            Build the first 15-minute box.
            H1 = max 1m high; L1 = min 1m low.

        09:45-10:00 ET
            Build the second 15-minute candle high H2. Make no decision before
            10:00 ET.

        At/after 10:00 ET
            opening_range = H1 - L1
            breakout_threshold = H1 + 0.20 * opening_range
            Setup is valid only if H2 > breakout_threshold. The second candle
            does NOT need to close above H1; only its HIGH is used.

        Entry
            Starting at 10:00 ET, inspect each completed 1-minute candle. Measure
            the distance from its [low, high] interval to H1. A candle that spans
            H1 has distance 0. A candle entirely above/below H1 has distance equal
            to its nearest edge. The FIRST candle with distance <= $0.05 qualifies.
            Simulated entry price is H1.

        Exit levels
            stop   = H1 - 0.25 * opening_range
            target = H1 + 2.00 * opening_range

        IEX remains the source of truth for the 1-minute bars.
        """
        ...
        # A ticker only becomes an S6 candidate if it has at least one IEX bar
        # in the first 15-minute window, exactly like the supplied reference code
        # requires a non-empty first candle.
        ...
        # First 15-minute candle: [09:30, 09:45).
        ...
        # Second 15-minute candle: [09:45, 10:00). Only HIGH matters for setup.
        ...
        # Do not decide until both 15-minute windows have finished.
        if m >= S6_SECOND_END and not q["evaluated"]:
            ...
            if (
                ...
                self._trace(
                    ...
                    "missing IEX data for one of the first two 15-minute candles; no S6 trade",
                    {"first_window_bars": q["n1"], "second_window_bars": q["n2"]},
                    once_key=f"{k}:incomplete",
                )
                return

            opening_range = float(q["h1"] - q["l1"])
            if opening_range <= 0:
                q["done"] = True
                self._trace(
                    6, "INVALID_BOX", b.s,
                    "first 15-minute opening range is zero/non-positive; no S6 trade",
                    {"range_high": q["h1"], "range_low": q["l1"]},
                    once_key=f"{k}:bad_box",
                )
                return

            breakout_threshold = float(q["h1"] + S6_BREAKOUT_RANGE_FRACTION * opening_range)
            if float(q["h2"]) <= breakout_threshold:
                q["done"] = True
                self._trace(
                    6, "IGNORE", b.s,
                    "second 15-minute high did not exceed H1 plus 20% of the opening range",
                    {
                        "range_high": q["h1"],
                        "range_low": q["l1"],
            ...
            q["armed"] = True
            q["opening_range"] = opening_range
            q["breakout_threshold"] = breakout_threshold
            self._trace(
                6, "BULLISH_SETUP", b.s,
                "second 15-minute high exceeded H1 plus 20% of opening range; waiting for first post-10:00 1-minute touch",
                {
                    "range_high": q["h1"],
                    "range_low": q["l1"],
                    "opening_range": opening_range,
                    ...
                    "second_15m_high": q["h2"],
                    "second_15m_low": q["l2"],
                    "touch_tolerance": S6_TOUCH_TOLERANCE,
                },
                once_key=f"{k}:armed",
            )

        # Exact reference-code entry test. Distance is zero when the 1m candle
        # touches/spans H1; otherwise it is the gap from the candle interval to H1.
        # The first qualifying candle from 10:00 onward is the entry candle.
        if q["armed"] and b.s not in self.pos[6] and m >= S6_SECOND_END:
            upper = float(q["h1"])
            low = float(b.l)
            high = float(b.h)
            distance = max(low - upper, 0.0) + max(upper - high, 0.0)
            if distance > S6_TOUCH_TOLERANCE:
                return

            opening_range = float(q.get("opening_range", q["h1"] - q["l1"]))
            entry = upper
            stop_distance = S6_STOP_BOX_FRACTION * opening_range
            target_distance = S6_TARGET_BOX_MULTIPLE * opening_range
            stop = entry - stop_distance
            target = entry + target_distance

            pos = self._entry(
                6, b.s, "LONG", entry, b.t,
                reason="TWO_15M_BREAKOUT_THEN_FIRST_1M_TOUCH",
                stop=stop,
                target=target,
                meta={
                    "range_high": q["h1"],
                    ...
                    "opening_range": opening_range,
                    "breakout_fraction_of_box": S6_BREAKOUT_RANGE_FRACTION,
                    "breakout_threshold": q.get("breakout_threshold", ""),
                    "touch_time_et": _et(b.t),
                    "touch_1m_open": b.o,
                    "touch_1m_low": b.l,
                    "touch_1m_high": b.h,
                    "touch_1m_close": b.c,
                    ...
                    "touch_tolerance": S6_TOUCH_TOLERANCE,
                    "box_height": opening_range,
                    "stop_fraction_of_box": S6_STOP_BOX_FRACTION,
                    "stop_distance": stop_distance,
                    "target_multiple_of_box": S6_TARGET_BOX_MULTIPLE,
...
def self_test():
    ...
    # S6 exact supplied logic.
    assert S6_FIRST_END == 585 and S6_SECOND_END == 600
    assert abs(S6_BREAKOUT_RANGE_FRACTION - 0.20) < 1e-12
    assert abs(S6_TOUCH_TOLERANCE - 0.05) < 1e-12
    assert abs(S6_STOP_BOX_FRACTION - 0.25) < 1e-12
    assert abs(S6_TARGET_BOX_MULTIPLE - 2.00) < 1e-12
    h1, l1 = 20.0, 19.2
    box = h1 - l1
    threshold = h1 + S6_BREAKOUT_RANGE_FRACTION * box
    assert abs(threshold - 20.16) < 1e-9
    assert not (20.16 > threshold)  # must be strictly greater
    assert 20.17 > threshold
    # Distance-to-H1 rule from the supplied reference code.
    def _dist(lo, hi, upper):
        return max(lo - upper, 0.0) + max(upper - hi, 0.0)
    assert _dist(19.99, 20.03, h1) == 0.0
    assert abs(_dist(20.04, 20.08, h1) - 0.04) < 1e-9
    assert abs(_dist(19.92, 19.96, h1) - 0.04) < 1e-9
    assert _dist(20.06, 20.08, h1) > S6_TOUCH_TOLERANCE
    entry = h1
    stop = entry - S6_STOP_BOX_FRACTION * box
    target = entry + S6_TARGET_BOX_MULTIPLE * box
    assert abs(stop - 19.8) < 1e-9 and abs(target - 21.6) < 1e-9
    ...
    print(
        ...
        "POC utility, S6 exact two-15m breakout-threshold + first-touch entry + 25%/200% box exits, strategy-specific sheet schemas, Day_end_price formula, stock filter"
    )
