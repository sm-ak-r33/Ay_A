...
class AI:
    ...
    def signal(self, s: str, direction: str, t: datetime, p: float, zr: float, zv: float, vol: float, fresh: bool = False):
        """Return a web-grounded YES/NO catalyst decision.

        The free router is intentionally allowed to choose its model. To avoid
        intermittent incompatibilities, this gate does not require JSON-schema
        output. It accepts only an explicit final result line, so arbitrary model
        chatter can never silently become a NO.
        """
        day = t.astimezone(NY)
        ...
        q = [
            {
                "role": "system",
                "content": (
                    "You are a strict real-time market-news catalyst gate. "
                    "You MUST use the web-search tool before deciding. "
                    "YES requires a recent, credible, company/security-specific event "
                    "that plausibly explains this exact abnormal move. Generic market "
                    "commentary, old articles, price-only pages, automated stock summaries, "
                    "and speculation are not catalysts. After searching, put the final "
                    "answer on the LAST LINE only, using exactly one of these formats:\n"
                    "NO\n"
                    "YES|five or six factual words\n"
                    "Do not put citations on the final line."
                ),
            },
            {
                "role": "user",
                "content": (
                    f"Ticker: {s}\n"
                    f"Direction: abnormal {move}\n"
                    f"Event time UTC: {t.isoformat()}\n"
                    f"Price: {p:.6f}\n"
                    f"z-range: {zr:.2f}\n"
                    f"z-volume: {zv:.2f}\n"
                    f"5-minute volume: {vol:.0f}\n"
                    f"Only accept catalysts published or announced within the prior {hrs} hours."
                ),
            },
        ]

        answer, err, src = self._call(
            "SIGNAL", s, q, AI_SIGNAL_MAX_TOKENS, True, response_format=None
        )
        ...
        if not answer:
            ...
            if raw.startswith("{") and raw.endswith("}"):
                try:
                    ...
                    hint = " ".join(str(obj.get("hint", "")).strip().split())
                    if dec in {"YES", "NO"}:
                        parsed = (dec, hint)
                except Exception:
                    parsed = None
            ...
            if parsed is None:
                ...
                final = lines[-1].strip("` ").strip() if lines else ""
                if final.upper().rstrip(".") == "NO":
                    parsed = ("NO", "")
                elif final.upper().startswith("YES|"):
                    hint = final.split("|", 1)[1].strip()
                    import re
                    hint = re.sub(r"\[[^\]]+\]\([^\)]+\)", "", hint)
                    hint = re.sub(r"https?://\S+", "", hint)
                    hint = " ".join(hint.split()[:6])
                    if hint:
                        parsed = ("YES", hint)

            if parsed is None:
                out = ("ERROR", "", src, "MALFORMED_SIGNAL_RESPONSE")
            else:
                dec, hint = parsed
                if dec == "NO":
                    hint = ""
    ...
    def ranking_reasons(self, syms: list[str]):
        ...
        q = [
            {
                "role": "system",
                "content": "Use current web results. Return one line per requested ticker: SYMBOL|4-5 word factual reason. If no verified reason, SYMBOL|No verified catalyst.",
            },
...
def live():
    ...
    if not ok:
        ...
        if _bool(cfg["ne"]):
            tg.send(f"[SYSTEM ERROR]\nAI_PREFLIGHT\nOpenRouter unavailable: {err[:300]}\nS1/S2/S3 news gates will not trade; S5 will still use its non-AI earnings fallback.")
